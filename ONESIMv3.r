...
### RUN BASECASE TO END 2012
	DIAG <- 2; OutMat1 <- OutMatInit <- OutMat
	Vcurrent <- V1950/sum(V1950)*InitPop[Setting]*10^6 
	for(t in 1:(62*12)) { print(c(t, ArtNdCov11)); Out <- timestep(Vcurrent,t,ArtNdCov11,DIAG,OutMat1)
		Vcurrent <- Out$Vnext; OutMatInit[t,] <- Out$Vout
		if(t==732) {ArtNdCov11 <- Out$Vout["ArtNdCov"]; print(OutMat[1,])  } } 
	V2012 <- Vcurrent 
